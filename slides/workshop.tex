...
\begin{columns}
  \begin{column}{.3\linewidth}
    \begin{center}
      {\large Looks good but no structure} \\
      \vspace{1cm}
    ...
    \begin{column}{.8\textwidth}
      \begin{figure} \centering
        \begin{tikzpicture}
          ...
          \node[block] (CONTEXT) at (-1,.5) {\textrm{\hologo{ConTeXt}}};
          \node[right] at (CONTEXT.east) {1996};
          ...
          \draw[thick, ->] (TEX) to[out=-90, in=90] (CONTEXT);
...
\begin{tabular}{>{\tt}l >{\tt}c >{\tt}c}
  \toprule
  \textsf{Environment} & \textrm{\LaTeX{}} & \textcolor{hsr-lightgrey}{\textrm{\TeX}}\footnote{Don't use it}\\
  \midrule
...
\begin{lstlisting}
Equation \ref{eqn:schroedinger} is the Schrödinger Equation that describes the evolution of a quantum state \(\psi\).
