...
\vspace{3mm}
\begin{lstlisting}[escapechar = ~]
\begin{thebibliography}{99} % widest label
...
%% Citations
\usepackage[
  backend = biber, % or bibtex (older sw.)
...
\begin{lstlisting}[font=\footnotesize, language = BibTeX]
...
\end{lstlisting}
\end{frame}
\begin{frame}[fragile]{BibTeX files (Example III)}
\begin{lstlisting}[font=\footnotesize, language = BibTeX]
@book{Griffith,
